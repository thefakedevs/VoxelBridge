...
public final class GeometryUtil {
    ...
    public static void applyOverlayOffset(float[] localPositions, int overlayIndex, float multiplier) {
        applyOverlayOffset(localPositions, overlayIndex, multiplier, 0f, 0f, 0f, false);
    }

    public static void applyOverlayOffset(float[] localPositions, int overlayIndex, float multiplier,
                                          float dirX, float dirY, float dirZ, boolean hasDir) {
        if (localPositions == null || localPositions.length < 12) {
            return;
        }

        final float overlayZFightOffset = 8e-4f;
        ...
        float offset = overlayZFightOffset * offsetMultiplier;
