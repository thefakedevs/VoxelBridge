...
public class FabricPlatformTextureHelper implements PlatformTextureHelper {
        ...
        if (glyphNormalized != null) {
            if (VoxelBridgeLogger.isDebugEnabled(LogModule.DYNAMIC_MAP)) {
                VoxelBridgeLogger.debug(LogModule.DYNAMIC_MAP,
                    "[FabricTextureHelper/1.21.1] Normalized glyph location: " + location + " -> " + glyphNormalized);
            }
        ...
        if (com.mojang.blaze3d.systems.RenderSystem.isOnRenderThread()) {
            ...
            if (dumpableImage.isPresent()) {
                if (VoxelBridgeLogger.isDebugEnabled(LogModule.DYNAMIC_MAP)) {
                    ...
                    VoxelBridgeLogger.debug(LogModule.DYNAMIC_MAP,
                        "[FabricTextureHelper/1.21.1] Dumpable texture loaded: " + location
    ...
    private static ResourceLocation normalizeDynamicGlyphLocation(ResourceLocation location) {
        ...
        // Glyph atlas pages are runtime textures in TextureManager (e.g. minecraft:default/0),
        // but RenderType may expose them as resource-like paths (textures/default/0.png).
