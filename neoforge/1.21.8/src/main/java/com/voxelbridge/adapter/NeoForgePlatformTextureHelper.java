...
public class NeoForgePlatformTextureHelper implements PlatformTextureHelper {
    ...
    public int getPixelRgba(NativeImage img, int x, int y) {
        if (img == null)
            return 0;
        // 1.21.8: getPixel returns ARGB, convert to ABGR expected by common pipeline.
    ...
    public Optional<NativeImage> readTexture(ResourceLocation location) {
        ...
        if (normalized != null) {
            ...
            if (VoxelBridgeLogger.isDebugEnabled(LogModule.DYNAMIC_MAP)) {
                VoxelBridgeLogger.debug(LogModule.DYNAMIC_MAP, "[NeoForgeTextureHelper/1.21.8] Normalized map location: " + location);
            }
        }
        ...
        if (com.mojang.blaze3d.systems.RenderSystem.isOnRenderThread()) {
            ...
            if (texture instanceof net.minecraft.client.renderer.texture.DynamicTexture dt) {
                ...
                if (pixels != null) {
                    if (VoxelBridgeLogger.isDebugEnabled(LogModule.DYNAMIC_MAP)) {
                        VoxelBridgeLogger.debug(LogModule.DYNAMIC_MAP, "[NeoForgeTextureHelper/1.21.8] DynamicTexture pixels loaded: " + location);
                    }
                    return Optional.of(copyNativeImage(pixels));
                }
            }

            // Note: HttpTexture is not available in 1.21.8, so we skip that check
        }
    ...
    private static void preheatMapTexture(ResourceLocation location) {
        ...
        try {
            ...
            if (data != null) {
                if (VoxelBridgeLogger.isDebugEnabled(LogModule.DYNAMIC_MAP)) {
                    VoxelBridgeLogger.debug(LogModule.DYNAMIC_MAP, "[NeoForgeTextureHelper/1.21.8] MapTextureManager.update: " + id);
                }
